<template>
  <div id="accessories">
    <div class="category-header mb-5">
      <h4>Accessories / {{ getSubCategory }}</h4>
      <swiper :options="swiperOption4" class="h-100">
        <swiper-slide class="slide-a1"></swiper-slide>
        <swiper-slide class="slide-a2"></swiper-slide>
        <swiper-slide class="slide-a3"></swiper-slide>
    ...
    <div class="row pt-5">
      <div class="col-md-2 d-none d-md-block">
        <ul class="category ml-3 ml-lg-5">
          <li>
            <a href="#" id="aNew" @click.prevent="onClickCategory('aNew')" 
            :class="{'active': getNowPage == 'aNew'}">
            <i class="fas fa-gift"></i>
            New Arrivals
            </a>
          </li>
          <li>
          <a href="#" id="gSale" @click.prevent="onClickCategory('aSale')" 
          :class="{'active': getNowPage == 'aSale'}"><i class="fas fa-smile-wink"></i> Sale</a>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    getProducts(page = 1){
        //console.log("Accessories.vue Methods - getProducts - Start");
        ...
        this.$http.get(api).then((response) => {
            ...
            vm.products = response.data.products.filter(item => item.category.indexOf('a') !== -1);
        })
        //console.log("Accessories.vue Methods - getProducts - End");
    },
    ...
    updateNowPage() {
      //console.log("Accessories.vue Methods - updateNowPage - Start");
      var vm = this;
      ...
      //console.log("Accessories.vue Methods - updateNowPage - End");
    },

    // Update NowPage with vuex when click category (Wesley)
    onClickCategory(id) {
        const vm = this;
        var nowPage = vm.$store.state.nowPage;
        let idStr = '#' + nowPage;
        vm.$router.push({ path: '/accessories', query: {id: id} }).catch(err => {});
        vm.updateNowPage();
    },

    openQuickViewModal(item){
      //console.log("Accessories.vue Methods - openQuickViewModal - Start");
      ...
      //console.log("Accessories.vue Methods - openQuickViewModal - End");
    },
  ...
  computed: {
    ...
    filterProducts(){
      //console.log("Accessories.vue Compute - filterProducts - Start");
      const vm = this;
      var nowPage = vm.$store.state.nowPage;
      this.getParams();
      //console.log("Accessories.vue Compute - filterProducts - End");
