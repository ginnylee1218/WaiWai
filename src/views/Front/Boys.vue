<template>
  <div id="boys">
    <div class="category-header mb-5">
      <h4>Boys / {{ getSubCategory }}</h4>
      <swiper :options="swiperOption3" class="h-100">
        <swiper-slide class="slide-b1"></swiper-slide>
        <swiper-slide class="slide-b2"></swiper-slide>
        <swiper-slide class="slide-b3"></swiper-slide>
    ...
    <div class="row pt-5">
      <div class="col-md-2 d-none d-md-block">
        <ul class="category ml-3 ml-lg-5">
          <li>
            <a href="#" id="bNew" @click.prevent="onClickCategory('bNew')" 
            :class="{'active': getNowPage == 'bNew'}">
            <i class="fas fa-gift"></i>New Arrivals
            </a>
          </li>
          <li>
            ...
            <ul class="category-level-2 ml-4">
              <li class="list-item" v-for="(item,index) in boysTops" :key="item.index">
                  <a :id="item.id" href="#" @click.prevent="onClickCategory(item.id)" 
          ...
          <li>
          <a href="#" id="bSale" @click.prevent="onClickCategory('bSale')" 
          :class="{'active': getNowPage == 'bSale'}">
          <i class="fas fa-smile-wink"></i> Sale</a>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    getProducts(page = 1){
        //console.log("Boys.vue Methods - getProducts - Start");
        ...
        this.$http.get(api).then((response) => {
            ...
            vm.products = response.data.products.filter(item => item.category.indexOf('b') !== -1);
        })
        //console.log("Boys.vue Methods - getProducts - End");
    },
    ...
    onClickCategory(id) {
        ...
        let idStr = '#' + nowPage;
        vm.$router.push({ path: '/boys', query: {id: id} }).catch(err => {});
        vm.updateNowPage();
    },

    openQuickViewModal(item){
      //console.log("Boys.vue Methods - openQuickViewModal - Start");
      ...
      //console.log("Boys.vue Methods - openQuickViewModal - End");
    },
